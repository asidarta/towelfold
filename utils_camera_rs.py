...
class CameraRS_Offline:
	...
	def get_depth_color_img(self, index):
		if index>=self.max_index:
			index = self.max_index-1
			print('[CameraRS_Offline]', self.serial_number, 'index exceeded')
			
		depth_img = cv2.imread(self.root+self.serial_number+'/depth/'+self.depth_filenames[index], cv2.IMREAD_ANYDEPTH)
		color_img = cv2.imread(self.root+self.serial_number+'/color/'+self.color_filenames[index])
		color_img = color_img[...,::-1] # Convert opencv BGR to open3d RGB

		return depth_img, color_img
	
	def get_point_cloud(self, index):
		return io.read_point_cloud(self.root+self.serial_number+'/'+self.ptcld_filenames[index])
